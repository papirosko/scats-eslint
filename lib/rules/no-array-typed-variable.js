...
module.exports = {
  ...
  create(context) {
    ...
    function classAllowsArrayTypes(classNode) {
      const nodesToCheck = [classNode];

      if (
        classNode.parent
        && (classNode.parent.type === 'ExportNamedDeclaration' || classNode.parent.type === 'ExportDefaultDeclaration')
      ) {
        nodesToCheck.push(classNode.parent);
      }

      return nodesToCheck.some((node) => sourceCode
        .getCommentsBefore(node)
        .some((comment) => comment.value.includes(ALLOW_ARRAY_TYPES_TAG)));
    }
