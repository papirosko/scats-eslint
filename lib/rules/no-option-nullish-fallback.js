...
const NONE_MESSAGE_ID = 'preferNoneOption';
...
function getOptionNullishWrap(node) {
  if (!node || node.type !== 'CallExpression' || node.arguments.length !== 1) {
    return null;
  }
  ...
  const kind = getNullishKind(node.arguments[0]);
...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      [NONE_MESSAGE_ID]: 'Use `none` instead of `option({{ kind }})`.',
    },
  },
  ...
  create(context) {
    ...
    return {
      CallExpression(node) {
        ...
        const optionNullishWrap = getOptionNullishWrap(node);

        if (optionNullishWrap) {
          context.report({
            node,
            messageId: NONE_MESSAGE_ID,
            data: {
              kind: optionNullishWrap.kind,
            },
            fix(fixer) {
              return fixer.replaceText(node, 'none');
            },
          });
          return;
        }
